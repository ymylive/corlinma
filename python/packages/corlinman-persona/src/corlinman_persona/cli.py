...
async def _run_decay(db_path: Path, config: DecayConfig) -> int:
    """Sweep all rows, apply decay, count how many changed.
...
async def _run_show(db_path: Path, agent_id: str) -> int:
    async with PersonaStore(db_path) as store:
...
async def _run_reset(db_path: Path, agent_id: str) -> int:
    async with PersonaStore(db_path) as store:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "decay-once":
        ...
        changed = asyncio.run(_run_decay(args.db, config))
        print(f"decay-once: {changed} row(s) changed")
        return 0
    if args.command == "show":
        return asyncio.run(_run_show(args.db, args.agent_id))
    if args.command == "reset":
        return asyncio.run(_run_reset(args.db, args.agent_id))
